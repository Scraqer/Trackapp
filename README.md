...
You'll need:
1. Your own VAPID keys (`npx web-push generate-vapid-keys`)
2. A Cloudflare account + Worker with a KV namespace bound as `SUBSCRIPTIONS`
3. Your VAPID private key added as a Cloudflare secret (`npx wrangler secret put VAPID_PRIVATE_KEY`)
4. Your VAPID public key pasted into both `worker.js` and `index.html` (has to match)
